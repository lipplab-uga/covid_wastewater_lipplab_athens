...
```{r}
        ...
        p1 <- p1 %>% plotly::add_trace(x = ~date, y = ~log_copy_per_L,
                                       ...
                                       text = ~paste('</br> Date: ', date,
                                                     ...
                                                     '</br> Target: ', target,
                                                     '</br> Log Copies/L: ', round(log_copy_per_L, digits = 2)),
                                       data = only_n1,
                                       symbol = ~Facility,
                                       yaxis = "y2",
                                       marker = list(color = '#1B9E77', size = 8, opacity = 0.65),
                                       showlegend = FALSE) %>%
          plotly::add_trace(x = ~date, y = ~log_copy_per_L,
                                       type = "scatter",
                                       mode = "markers",
                                       hoverinfo = "text",
                                       text = ~paste('</br> Date: ', date,
            ...
            layout(yaxis2 = list(overlaying = "y", 
                                 ...
                                 range = c(0, 8), showline = TRUE,
                                 automargin = TRUE)) %>%
            layout(legend = list(orientation = "h", x = 0.2, y = -0.3))
        
        p1 <- p1 %>% plotly::add_segments(x = as.Date("2020-03-14"), xend = ~max(date + 10), y = 35.5, yend = 35.5,
                                          opacity = 0.35,
                                          line = list(color = "black", dash = "dash")) %>%
          layout(annotations = list(x = as.Date("2020-03-27"), y = 38, xref = "x", yref = "y", 
